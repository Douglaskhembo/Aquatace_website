...
function CheckoutPage() {
  ...
  if (lines.length === 0) {
    return (
      <div className="container-page py-16">
        <EmptyState title="No items to checkout" description="Add some products first."
          cta={<Button asChild className="rounded-full"><Link to="/products">Shop products</Link></Button>} />
      </div>
    );
  }
  ...
  const validate = (): boolean => {
    ...
    if (form.email && !/^\S+@\S+\.\S+$/.test(form.email.trim())) e.email = "Enter a valid email or leave blank.";
    ...
    try {
      const res = await submit({
        data: {
          ...
          delivery: { address: location.address, lat: location.lat, lng: location.lng },
  ...
  return (
    <div className="container-page py-10 md:py-14">
      <Button asChild variant="ghost" size="sm" className="mb-4 rounded-full"><Link to="/cart"><ArrowLeft className="mr-1 h-4 w-4" /> Back to cart</Link></Button>
      <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">Checkout</h1>
      <p className="mt-2 max-w-xl text-muted-foreground">Enter your delivery details. We'll route your order to the nearest branch automatically.</p>
          ...
          <Card className="rounded-3xl border-border/60 shadow-[var(--shadow-soft)]">
            <CardContent className="grid gap-5 p-6 md:p-8">
              ...
              <Field id="name" label="Full name" error={errors.name}>
                <Input id="name" value={form.name} onChange={(e) => update("name", e.target.value)} placeholder="Jane Wanjiku" autoComplete="name" required />
              </Field>
              <div className="grid gap-5 md:grid-cols-2">
                <Field id="phone" label="Phone number" error={errors.phone}>
                  <Input id="phone" type="tel" value={form.phone} onChange={(e) => update("phone", e.target.value)} placeholder="+254 712 345 678" autoComplete="tel" required />
                </Field>
              ...
              <Field id="notes" label="Additional notes (optional)">
                <Textarea id="notes" value={form.notes} onChange={(e) => update("notes", e.target.value)} placeholder="Landmarks, gate code, preferred time…" rows={3} />
              </Field>
            </CardContent>
          </Card>
          ...
          <Card className="rounded-3xl border-border/60 shadow-[var(--shadow-soft)]">
            <CardContent className="grid gap-5 p-6 md:p-8">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">Search, use your current location, or drop a pin on the map.</p>
              </div>
              <DeliveryMap value={location} onChange={setLocation} />
              {errors.location && <p className="text-xs text-destructive" role="alert">{errors.location}</p>}
            </CardContent>
          </Card>
        ...
        <aside className="lg:sticky lg:top-24 lg:self-start">
          <Card className="rounded-3xl border-border/60 shadow-[var(--shadow-soft)]">
            <CardContent className="p-6">
                ...
                {lines.map((l) => (
                  <li key={l.product.id} className="flex items-center justify-between gap-3 py-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">{l.qty} × {formatKES(l.product.price)}</p>
                    </div>
                    <span className="text-sm font-semibold">{formatKES(l.product.price * l.qty)}</span>
                  </li>
                ))}
              </ul>
              <div className="mt-4 space-y-2 border-t border-border pt-4 text-sm">
                <div className="flex justify-between"><span className="text-muted-foreground">Subtotal</span><span className="font-medium">{formatKES(subtotal)}</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">Delivery</span><span className="font-medium">On confirmation</span></div>
                {assigned && (
                  <div className="flex justify-between"><span className="text-muted-foreground">Branch</span><span className="font-medium">{assigned.name}</span></div>
                )}
              </div>
              ...
              <Button type="submit" size="lg" className="mt-6 h-12 w-full rounded-full bg-[#25D366] text-base text-white hover:bg-[#1ebe57]" disabled={submitting || !location}>
                {submitting ? "Placing order…" : <><CheckCircle2 className="mr-2 h-5 w-5" /> Send order via WhatsApp</>}
              </Button>
