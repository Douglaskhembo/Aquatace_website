...
import { ChevronRight, Heart, Minus, Plus, ShoppingCart, MessageCircle, Truck, ShieldCheck, RotateCcw } from "lucide-react";
  ...
  return (
    <div className="container-page py-8 md:py-12">
      <nav className="flex items-center gap-1 text-xs text-muted-foreground" aria-label="Breadcrumb">
        ...
        <Link to="/category/$slug" params={{ slug: p.category }} className="hover:text-foreground capitalize">{p.category}</Link>
        <ChevronRight className="h-3 w-3" />
      ...
      <div className="mt-6 grid gap-10 lg:grid-cols-2">
        ...
        <div>
          <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }}
            ...
            <ProductImage category={p.category} label={p.size} image={p.image} size="lg" className="h-full w-full" />
          </motion.div>
        ...
        <div>
          <div className="flex items-center gap-2">
            <Badge variant="secondary" className="rounded-full">{p.brand}</Badge>
          ...
          <div className="mt-8 flex items-center gap-3">
            <div className="flex items-center rounded-full border border-border">
              <Button variant="ghost" size="icon" className="h-11 w-11 rounded-full" onClick={() => setQty((q) => Math.max(1, q - 1))} aria-label="Decrease quantity"><Minus className="h-4 w-4" /></Button>
              <span className="w-8 text-center text-sm font-semibold" aria-live="polite">{qty}</span>
              <Button variant="ghost" size="icon" className="h-11 w-11 rounded-full" onClick={() => setQty((q) => q + 1)} aria-label="Increase quantity"><Plus className="h-4 w-4" /></Button>
            </div>
            <Button size="lg" className="h-12 flex-1 rounded-full text-base"
              onClick={() => { add(p, qty); toast.success("Added to cart", { description: `${qty} × ${p.name}` }); }}>
              <ShoppingCart className="mr-2 h-4 w-4" /> Add to cart
            </Button>
            <Button size="icon" variant="outline" className="h-12 w-12 rounded-full" aria-label="Add to wishlist" onClick={() => toast.success("Saved to wishlist")}>
              <Heart className="h-4 w-4" />
            </Button>
          </div>

          <Button asChild size="lg" variant="outline" className="mt-3 h-12 w-full rounded-full">
            <a href={waLink(`Hi Aquatace, I'd like to order ${qty} × ${p.name} (${formatKES(p.price)}).`)} target="_blank" rel="noreferrer">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {[
              ...
              <div key={f.t} className="flex items-start gap-3">
                <div className="grid h-9 w-9 place-items-center rounded-xl bg-accent text-primary"><f.Icon className="h-4 w-4" /></div>
                <div>
          ...
          <div className="mt-8">
            <Tabs defaultValue="specs">
              <TabsList className="rounded-full">
                ...
                <TabsTrigger value="delivery" className="rounded-full">Delivery</TabsTrigger>
              </TabsList>
              ...
              <TabsContent value="delivery" className="mt-4 text-sm text-muted-foreground">
                Orders placed before 5 PM are delivered the same day within Nairobi. Nationwide delivery via courier
